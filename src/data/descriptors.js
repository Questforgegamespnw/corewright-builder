/* =========================
   FORM DESCRIPTORS
========================= */

export const FORM_DESCRIPTORS = {
  brawler:
    "broad-shouldered heavy construct, reinforced torso, oversized forearms, low center of gravity, brutal melee frame",

  predator:
    "low aggressive bestial construct, digitigrade limbs, lunging hunter posture, sleek pursuit anatomy",

  bulwark:
    "towering defensive construct, thick torso, shield-like proportions, immovable guard stance",

  strider:
    "long-limbed agile construct, fast skirmisher proportions, extended stride geometry",

  artillery:
    "precision siege construct with elevated firing limbs, stabilized chassis, ranged projection apparatus",

  climber:
    "multi-jointed construct with hooked limbs, spider-like articulation, wall-scaling anatomy",

  aquatic:
    "sealed amphibious construct with finned limbs, pressure-stable body, fluid motion design",

  glider:
    "lightweight aerial construct with finned or membranous extensions, glide-capable frame, dive-strike posture",

  constrictor:
    "heavy grappling construct, crushing limbs, locking joints, clamp-like appendages, restraint-focused melee frame"
};

/* =========================
   FORM DESCRIPTORS (SAFE)
========================= */

export const FORM_DESCRIPTORS_SAFE = {
  brawler:
    "broad-shouldered heavy construct, reinforced torso, oversized forearms, low center of gravity, sturdy mechanical silhouette",

  predator:
    "low, agile bestial construct, digitigrade limbs, streamlined motion, sleek pursuit-inspired anatomy",

  bulwark:
    "towering defensive construct, thick torso, shield-like proportions, grounded guardian stance",

  strider:
    "long-limbed agile construct, fast skirmisher proportions, extended stride geometry",

  artillery:
    "precision arcane construct with elevated projection limbs, stabilized chassis, refined ranged silhouette",

  climber:
    "multi-jointed construct with hooked limbs, spider-like articulation, wall-scaling anatomy",

  aquatic:
    "sealed amphibious construct with finned limbs, pressure-stable body, fluid motion design",

  glider:
    "lightweight aerial construct with finned or membranous extensions, glide-capable frame, graceful descending posture",

  constrictor:
    "heavy control construct, reinforced limbs, locking joints, clamp-like appendages, restraint-focused melee frame"
};

/* =========================
   TEMPLATE DESCRIPTORS
========================= */

export const TEMPLATE_DESCRIPTORS = {
  stone:
    "carved stone plates, weathered mineral texture, heavy mass, glowing runic seams, immense structural weight",

  clay:
    "smooth earthen body, sculpted artisan form, cracked surfaces that subtly reform and self-repair",

  wood:
    "layered hardwood construction, visible grain patterns, flexible joints, druidic carved details",

  metal:
    "plated steel and brass armor, riveted chassis, forged industrial construction, reinforced plating",

  cloth:
    "animated woven fabric body, flowing draped structure, ethereal and uncanny movement",

  bone:
    "jagged ivory framework, skeletal construction, sharp protrusions and predatory lines",

  blood:
    "semi-translucent crimson body, fluid internal motion, glowing alchemical channels, unstable organic form"
};

/* =========================
   TEMPLATE DESCRIPTORS (SAFE)
========================= */

export const TEMPLATE_DESCRIPTORS_SAFE = {
  stone:
    "carved stone plates, weathered mineral texture, heavy mass, glowing runic seams, immense structural weight",

  clay:
    "smooth earthen body, sculpted artisan form, cracked surfaces that subtly reform and self-repair",

  wood:
    "layered hardwood construction, visible grain patterns, flexible joints, druidic carved details",

  metal:
    "plated steel and brass armor, riveted chassis, forged industrial construction, reinforced plating",

  cloth:
    "animated woven fabric body, flowing draped structure, ethereal and uncanny movement",

  bone:
    "jagged ivory framework, skeletal construction, elegant sharp contours and a striking silhouette",

  blood:
    "semi-translucent crimson body, fluid internal motion, glowing alchemical channels, mysterious organic form"
};

/* =========================
   ENGINE DESCRIPTORS
========================= */

export const ENGINE_DESCRIPTORS = {
  none:
    "a faint neutral arcane core with minimal glow, stable and inactive energy signature",

  flame:
    "a furnace-like core glowing with internal fire, ember-lit seams, heat distortion, smoke vents, burning aura",

  frost:
    "a cold radiant core with icy glow, frost forming along edges, vapor trails and crystalline buildup",

  storm:
    "a crackling lightning core, arcs of electricity dancing across limbs, ionized glow and charged air distortion",

  aether:
    "a floating arcane core suspended within the chest, glowing glyphs, soft radiant energy and levitation effects",

  earth:
    "a dense geode-like core, pulsing with earthen energy, dust shockwaves and grounded power radiating outward"
};

/* =========================
   ENGINE DESCRIPTORS (SAFE)
========================= */

export const ENGINE_DESCRIPTORS_SAFE = {
  none:
    "a faint neutral arcane core with minimal glow, stable and inactive energy signature",

  flame:
    "a radiant core glowing with internal fire, ember-lit seams, warm heat shimmer, soft smoke vents, luminous aura",

  frost:
    "a cold radiant core with icy glow, frost forming along edges, vapor trails and crystalline buildup",

  storm:
    "a crackling lightning core, arcs of electricity dancing across limbs, ionized glow and charged air distortion",

  aether:
    "a floating arcane core suspended within the chest, glowing glyphs, soft radiant energy and levitation effects",

  earth:
    "a dense geode-like core, pulsing with earthen energy, drifting dust and grounded power radiating outward"
};

/* =========================
   INFUSION DESCRIPTORS
========================= */

export const INFUSION_DESCRIPTORS = {
  reinforced_frame:
    "thickened armor plating, reinforced structural ribs, heavy support struts built for durability",

  war_construct:
    "dual combat armatures, weapon-integrated limbs, aggressive targeting posture, combat-optimized design",

  dexterous_manipulators:
    "articulated hands with precision fingers, fine motor joints, tool-ready appendages",

  accelerated_servos:
    "exposed speed pistons, sleek leg assemblies, kinetic joint design built for rapid movement",

  reactive_plating:
    "layered segmented armor, shifting defensive plates, impact-dampening surface structure",

  adaptive_plating:
    "morphing armor layers that shift and reconfigure dynamically in response to threats",

  arcane_conduit:
    "glowing magical channels running from the core through the limbs, energy conduits for spell delivery",

  anchored_frame:
    "low-set stabilizers, reinforced base, grounding spikes and heavy stance architecture",

  overcharged_core:
    "intensely glowing unstable core, energy leaking through seams, volatile power surges",

  sentinel_protocol:
    "shield-like armatures, defensive sensor arrays, intercept-ready stance geometry",

  gravitic_core:
    "orbiting arcane sigils, distortion field around the body, subtle gravity warping effects",

  self_repair_matrix:
    "cracked surfaces actively reforming, glowing repair glyphs, self-mending structure",

  siege_engine:
    "massive reinforced limbs, breaching hammer-like arms, heavy impact-focused construction",

  reflexive_countermeasures:
    "adaptive armor segments, reactive shutters, anticipatory defensive mechanisms",

  predictive_guard_matrix:
    "threat-analysis lenses, layered defensive shutters, forecasting sigils, anti-barrage plating",

  sundering_strikes:
    "stress-fracture targeting runes, armor-cracking impact points, precision breach geometry",

  adamantine_plating:
    "dark adamantine armor plates, reinforced vital seams, catastrophic-failure-resistant chassis",

  cognitive_matrix:
    "advanced head structure with glowing eye cluster, elegant arcane lattice of intelligence",

  phase_shifter:
    "partial translucence, ghosted limb edges, flickering reality distortion around the frame",

  overdrive_protocol:
    "overclocked vents, stressed glowing internals, aggressive power posture and heat output",

  replication_matrix:
    "mirrored core shards, echo projections, faint duplicate silhouettes forming near the body",

  elemental_convergence:
    "engine-aligned resonance lines, amplified elemental vents, intensified core glow",

  colossus:
    "oversized monumental frame, immense limbs, towering proportions, siege-scale construction"
};

/* =========================
   INFUSION DESCRIPTORS (SAFE)
========================= */

export const INFUSION_DESCRIPTORS_SAFE = {
  reinforced_frame:
    "thickened armor plating, reinforced structural ribs, heavy support struts built for durability",

  war_construct:
    "dual reinforced armatures, integrated mechanical limbs, advanced targeting design, action-focused construction",

  dexterous_manipulators:
    "articulated hands with precision fingers, fine motor joints, tool-ready appendages",

  accelerated_servos:
    "exposed speed pistons, sleek leg assemblies, kinetic joint design built for rapid movement",

  reactive_plating:
    "layered segmented armor, shifting defensive plates, impact-dampening surface structure",

  adaptive_plating:
    "morphing armor layers that shift and reconfigure dynamically in response to threats",

  arcane_conduit:
    "glowing magical channels running from the core through the limbs, energy conduits for spell delivery",

  anchored_frame:
    "low-set stabilizers, reinforced base, grounding spikes and heavy stance architecture",

  overcharged_core:
    "intensely glowing unstable core, energy leaking through seams, volatile power surges",

  sentinel_protocol:
    "shield-like armatures, defensive sensor arrays, intercept-ready stance geometry",

  gravitic_core:
    "orbiting arcane sigils, distortion field around the body, subtle gravity warping effects",

  self_repair_matrix:
    "cracked surfaces actively reforming, glowing repair glyphs, self-mending structure",

  siege_engine:
    "massive reinforced limbs, breaching hammer-like arms, heavy impact-focused construction",

  reflexive_countermeasures:
    "adaptive armor segments, reactive shutters, anticipatory defensive mechanisms",

  predictive_guard_matrix:
    "threat-analysis lenses, layered defensive shutters, forecasting sigils, anti-barrage plating",

  sundering_strikes:
    "stress-fracture targeting runes, armor-weakening impact points, precision breach geometry",

  adamantine_plating:
    "dark reinforced armor plates, protected vital seams, resilient heavy chassis",

  cognitive_matrix:
    "advanced head structure with glowing eye cluster, elegant arcane lattice of intelligence",

  phase_shifter:
    "partial translucence, ghosted limb edges, flickering reality distortion around the frame",

  overdrive_protocol:
    "overclocked vents, stressed glowing internals, aggressive power posture and heat output",

  replication_matrix:
    "mirrored core shards, echo projections, faint duplicate silhouettes forming near the body",

  elemental_convergence:
    "engine-aligned resonance lines, amplified elemental vents, intensified core glow",

  colossus:
    "oversized monumental frame, immense limbs, towering proportions, siege-scale construction"
};

/* =========================
   ART STYLES
========================= */

export const ART_STYLES = {
  sheet:
    "Present the construct like polished tabletop concept art on a clean neutral background, full-body, centered composition, highly readable silhouette, professional fantasy illustration.",

  portrait:
    "Portrait-style fantasy illustration focused on the upper body and head, dramatic framing, strong lighting, expressive core glow, detailed material rendering.",

  action:
    "Dynamic action scene, cinematic fantasy illustration, the construct in motion, dramatic perspective, energy effects, environmental interaction, highly dynamic pose.",

  blueprint:
    "Arcane blueprint presentation, technical concept sheet, clean orthographic feel, design-study presentation, visible construction logic, labeled visual language, polished fantasy design document aesthetic."
};

/* =========================
   ART STYLES (SAFE)
========================= */

export const ART_STYLES_SAFE = {
  sheet:
    "Present the construct like polished tabletop concept art on a clean neutral background, full-body, centered composition, highly readable silhouette, professional fantasy illustration.",

  portrait:
    "Portrait-style fantasy illustration focused on the upper body and head, dramatic framing, strong lighting, expressive core glow, detailed material rendering.",

  action:
    "Dynamic fantasy scene, cinematic illustration, the construct in motion, dramatic perspective, magical energy effects, environmental interaction, highly dynamic pose.",

  blueprint:
    "Arcane blueprint presentation, technical concept sheet, clean orthographic feel, design-study presentation, visible construction logic, labeled visual language, polished fantasy design document aesthetic."
};